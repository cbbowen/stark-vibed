...
| [brush.md](docs/brush.md) | §6.1, §6.2, §6.6, §6.9, §6.11, §6.12, §6.13 | Tiles and channels, path fitting, swept-segment stamping, the wet-mixing dynamics loop, brush shape assets, drag-and-hold shape assist, stroke smoothing (the towed tip), the eraser, the liquify warp |
| [rendering.md](docs/rendering.md) | §6.3–§6.5, §6.7, §6.10 | The three compositing passes, blend modes, the media/lighting pass, aprons and the canvas substrate, Oklab and Mixbox, the HDR output — the display as a view setting, and why an export never reads it — the generated uniform mirrors |
...
| [filters.md](docs/filters.md) | §21 | Filter layers: adjustment as a layer, where it sits *is* its scope, the color filter, the spectral chromatic aberration, the gradient map, the FFT focal blur |
...
crates/
  ...
  stark-ui/    the **frontend's model** (§11.2): what a chrome is written in, below
                   ...
                   with only `run`, `active` and the mark left to each frontend. Names no `dioxus::`, `wgpui::`,
