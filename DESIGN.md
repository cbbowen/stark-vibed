...
| — | Transform ([TRANSFORM_DESIGN.md](TRANSFORM_DESIGN.md)) | **done (first cut)** — `ActionKind::Transform`, the parcel/combine/mask GPU passes, exactness + replay tests, and the gesture UI (resize handles + rotate knob + flips/Done bar, lossless preview through the same renderer); skew chrome + clipboard remain |
