...
/// ground that would not resolve, and a ground that does not resolve is `Flat`, whose
...
pub fn settle(engine: &mut stark_core::Engine, file: &stark_core::DocumentFile) {
    for need in engine.unresolved_content(file) {
        ...
        let installed = match need.surface() {
            Some(id) => engine.accept_surface(id, &bytes).map(|_| ()),
