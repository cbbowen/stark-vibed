...
fn folding_history_does_not_change_what_a_peer_is_sent() {
    ...
    let before = pixels(&mut engine);
    engine.start_collaboration(stark_core::document::ActorId(7));
...
fn a_shared_session_never_folds() {
    ...
    engine.start_collaboration(stark_core::document::ActorId(3));
    engine.process(ViewCommand::SetHistoryBudget(0));
    paint_a_session(&mut engine, 48);
    ...
    let file = stark_core::DocumentFile::from_bytes(&log).expect("decode");
    assert_eq!(
        file.actions.len(),
        48,
