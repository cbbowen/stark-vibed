...
//!
//! Both are **requests** rather than commands — they have to answer (§4) — and both
//! go through the *same* draw list rendering does
//! ([`Engine::composite_groups`]), so what they report is the paint the screen is
//! drawing rather than a second opinion about it. Neither samples the media pass's
//! output: that lights the paint, tonemaps it and encodes sRGB, so picking it up
//! would load the brush with a color the palette never mixed.
//!
...
pub enum PickSource {
    ...
    /// be a color no layer holds — matching what the eye sees at a point rather than
    /// what is stored there, which is the question asked when the paint is thin.
    ...
    /// source is asked to ignore — none of them says what the paint **is**, so turning
    /// a layer down reports the same color rather than a paler one.
    ///
    /// **Except at zero**, where the layer contributes nothing at all and this answers
    /// `None` like bare canvas: every setting above zero is a layer that is there and
    /// turned down, and zero is a layer that is switched off.
    Layer(LayerId),
    ...
    /// argument: they say how the group meets what is beneath it, which is
    /// exactly what this source excludes. For a layer in the root stack the
    ...
    /// [`CompositeOverSubstrate`](Self::CompositeOverSubstrate), so bare canvas
    /// answers with the substrate.
    Below(LayerId),
}
...
/// divide by. What they divide by — the summed opacity or the count — is the whole
/// of what separates the two pick modes.
...
/// is no paint in it.
///
/// The composite is premultiplied by opacity (§6.1), so dividing the sum by the
/// summed opacity *is* the opacity-weighted mean: a texel carrying more paint counts
/// for more and a bare one counts for nothing. That is what lets a radius wider than
/// the stroke still report the stroke's color rather than a wash of it.
...
/// composite's premultiplied color standing in for `c·a`.
///
/// A **plain** mean where [`mean_channels`] is an opacity-weighted one, and that
/// difference *is* the mode: with the substrate behind it every texel is opaque, so a
/// patch half-covered by a stroke reads as the mixture of paint and canvas an eye
...
///
/// Snapped to the pixel rather than centred on the point itself, because pass A
/// samples tile textures bilinearly: a fractional offset would blend neighbouring
/// texels and answer with a color that is at neither. Snapping puts every fragment on
/// a texel centre, so radius 0 reports exactly the texel under the cursor.
///
...
/// [`ViewTransform::visible_tiles`]'s own contract carried across the batch: an
/// optimization that cannot see one of its inputs has to do nothing about all of
/// them. Folding the *bounds* instead would let a non-finite point vanish into a
/// `min`/`max` and quietly shrink the answer.
...
impl Engine {
    ...
    /// rather than joining [`InputCommand`](crate::command::InputCommand), whose whole
    /// property is that nothing comes back (§4).
    ///
    /// What it samples is the **raw layer channels**, not the composited, *lit*
    /// result the screen shows. The media pass lights the paint, tonemaps it and
    /// encodes sRGB, so picking its output would load the brush with a color the
    /// palette never mixed — and in a Mixbox document (§6.7) with a display color
    /// rather than the pigment mixture, which would make picking the mix back up
    /// impossible.
    ///
    ...
    ///
    /// The path — canvas-space, as traced — is resampled evenly by arc length
    /// ([`stark_model::gradient::resample`]), each sample is picked exactly as
    /// [`Engine::pick_color`] picks (same sources, same patch mean, same
    /// raw-channels-not-lit rule — in a Mixbox document the ramp is of pigment
    /// mixtures), and [`stark_model::gradient::fit`] reduces the run to the fewest
    /// stops that reproduce it within a perceptual tolerance.
    ///
    ...
    pub(crate) fn pick_colors(
        ...
        // Read here rather than in the future, which deliberately does not borrow the
        // engine. The substrate rides the *source* rather than a flag beside it:
        // asking paint for its own color and asking what the canvas shows are
        // different questions, and only the second kind has a substrate in it.
        ...
        // rather than to each in turn (§6.3): only the cull varies with the point, so a
        // list per sample would be a hundred-odd tree walks for one answer. A union is
        // the *wider* cull, which is sound in the only direction that matters — a cull
        // may name tiles a pass then draws nothing for, never omit one it needed (see
        // `ViewTransform::visible_tiles`).
        //
        ...
        // Each patch is its own view of the same document, and the views are slots
        // (`composite::ViewBindings`), so what orders them is the offset a patch binds
        // and the whole trace is one encoder rather than a round trip per sample.
        ...
        // same cull, so this is one plan, and it has to be one *upload*: what the
        // streams write into is one buffer apiece on the compositor
        // (`Compositor::prepare_pick`). Kept across the loop and past the submit.
        //
        ...
        async move {
            ...
            // "nothing here" — what this answers for bare canvas — is the honest
            // answer for a device that can no longer be read. One `None` per point, so
            // the alignment callers zip against holds whatever happened.
            // `ObservableState::gpu_failure` is what *reports* the failure.
            ...
            texel_sets
                ...
                .map(|(i, texels)| {
                    ...
                    // The residual takes the **same** two means: its target's alpha
                    // duplicates the color's, so `sum[3]` is the same coverage sum
                    // either way and both weightings are already right for it (§6.7).
    ...
    ///
    /// Shared by both pick paths. `color` and `resid` are read back (the eyedropper
    /// wants both; a pigment space's residual is just more colour channels), so both
    /// get `COPY_SRC`. `aux` never is — pass A writes it and the height it accumulates
    /// says how *much* paint is there, not what colour it is — so it gets none.
    ///
...
///
/// Not [`PICK_MIN_OPACITY`]'s near-zero floor, which asks whether there is enough
/// paint to divide by without amplifying float noise into a hue. This asks whether
/// *this is the layer the artist is pointing at*, so the threshold sits where a texel
/// starts to read as covered rather than where it stops being empty.
///
...
/// it, which the single-layer branch of `composite_groups` cannot do: it asks only
/// about the layer named, which is right for "sample this layer" (§18.0.2) and wrong
/// here, because the members of a hidden group are not on the screen to be pointed at.
...
impl Engine {
    ...
    /// consequences a user can notice: a layer turned down to 10% still answers
    /// where its paint is solid, and a **clipped** layer answers over its whole
    /// extent rather than only where the paint beneath it lets it through (§14.4).
    ///
    ...
    pub fn pick_layer(
        ...
        // tile origins (`Compositor::prepare_pick`).
        self.compositor
