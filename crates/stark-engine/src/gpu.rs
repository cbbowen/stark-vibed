...
//!
//! **The `pub use` list below is what the *crate* reaches across these module
//! boundaries** — `gpu` is itself `pub(crate)` (see [`crate`]), so nothing here
//! leaves the crate by this route. The modules are `pub(crate)` too, so a `pub` item
//! inside one is visible to the rest of the engine only once it is named here —
//! which is the point: what crosses should be a decision, not a consequence of how a
//! file happened to be split.
//!
...
// A name earns a line here by being reached from *another* module. One re-exported
// for a public path nobody outside took, while the crate itself reached it through
// its own module, had a reader at neither end — which is what has taken names off
// this list, and the test to apply before adding one.
...
/// both ends of that — [`TilePairHandle::read_channels`](tile::TilePairHandle::read_channels) which mints one and
