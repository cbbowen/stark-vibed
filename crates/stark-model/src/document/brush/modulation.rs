//! The pen mappings (§6.2): what a [`Modulation`] reads off the pen, the response it
//! shapes that into, and the five records naming which of a brush's knobs one may be
//! pointed at.
//!
...
/// What a [`Modulation`] reads off the pen (§6.2). Both are already carried per point
/// of the fitted curve ([`ControlPoint`](crate::path::ControlPoint)) and interpolated
/// per swept segment, so a source costs the renderer nothing to evaluate or store.
...
pub enum ModSource {
    ...
    /// [`floor`](Modulation::floor) for the whole stroke — a brush meant to be usable
    /// without a tablet says so by leaving the floor off zero.
    Tilt,
}
...
/// guarantee the rest of the engine derives from `BrushParams` — the frozen-span rule's
/// radius bound, the region fit, the choice of render path, the flattener's exchange
/// step — is stated against the brush's own numbers and stays sound knowing nothing of
/// modulation. A remap that could also scale *up* would put a correction into every one
/// of those places, and a missed one renders differently live and committed (§1.3).
///
...
/// Clamp on [`Modulation::curve`]'s underlying bias, and so on how steep a response can
/// be: the factor's slope reaches `(1 − k)/k` at one end of the input and `k/(1 − k)` at
/// the other, which at 0.1 is **9**.
///
/// A bound rather than taste: a segment sweeps at *one* value of every parameter, so a
/// steep response is paid for in segments — the flattener divides its attribute budget
...
impl Modulation {
    ...
    /// replay, goldens and peers have to agree on it to the last bit (§12.1): IEEE-754
    /// pins `+ − × ÷` to a correctly-rounded result, where `powf` is not specified at
    /// all.
    ...
    /// The bias the response and its slope bound are both written in, from the `[-1, 1]`
    /// knob the UI shows: `curve` mapped affinely onto `[0, 1]` and held off the ends by
    /// `MIN_BIAS`.
    ///
    /// One function rather than the expression twice, because [`bias`](Self::bias) and
    /// [`max_slope`](Self::max_slope) have to agree: the flattener buys segments against
    /// the slope this pins, and an under-estimate is the staircase `MIN_BIAS` exists to
    /// prevent.
    ///
    /// `curve = 0` lands on `k = 0.5` and so on `m = 0`, every step of which is exact in
    /// binary — which makes the linear case unconditional rather than lucky.
    fn k(&self) -> f32 {
    ...
    ///
    /// Changes no pixel — [`factor`](Self::factor) and [`max_slope`](Self::max_slope)
    /// already clamp what they read — but keeps what is *stored* to what a slider can
    /// show, so a `NaN` curve cannot reach a panel.
...
/// The steepest response across a set of targets — how much finer the path has to be
/// flattened for a modulated ramp to stay smooth (see `MIN_BIAS`). 1 for the unmodulated
/// brush *and* for the plain linear mappings, so the everyday brush flattens on the
...
///
/// They stay five distinct types — each is its own save-format record, and "a mapping
/// cannot name a knob its brush does not have" is what having five of them buys. The
/// arithmetic over those targets is a pure function of [`all`](Self::all).
///
/// **[`all`](Self::all) and [`from_all`](Self::from_all) are inverses, and both are
/// exhaustive.** A pattern with no `..` and a literal with no `..` are each refused by
/// the compiler once a target is added to the struct, so a new target has to be listed
/// in two places before this builds. Reading the fields one by one would leave it
/// quietly missing from [`max_slope`](Self::max_slope), and an under-estimated slope is
/// not an error anywhere — it is a modulated ramp drawn as a staircase.
///
/// `N` is the trait's parameter rather than an associated const because an associated
/// const cannot be an array length without `generic_const_exprs`.
...
/// its size, its elongation, and how far it settles into the substrate's tooth. An
/// effect's own rates are mapped *with* the effect ([`PaintModulations`],
...
/// segment carries one size, one set of rates and one orientation (§6.6). `hardness`
/// (baked per value into the prefix-τ texture) and `charge` (an initial condition, not
/// a rate) cannot be modulated at all.
///
/// `None` on a target is not "a modulation with no effect" — it is skipped entirely, so
/// the parameter reaches the renderer as the exact float the slider holds.
...
pub struct BrushModulations {
    ...
    /// Scales [`ToothParams::give`](super::ToothParams::give) — how far the tool settles
    /// into the canvas's substrate (§6.4). Mapped to pressure this is the charcoal: a
    /// light touch prints the peaks alone, and borne down the grain fills in.
    pub tooth_give: Option<Modulation>,
    /// Scales [`BrushParams::stretch`](super::BrushParams::stretch) — how far the extent
    /// elongates along the brush's facing axis (§6.6). Mapped to [`ModSource::Tilt`] with
    /// [`OrientationSource::Pen`](super::OrientationSource::Pen) this is the pencil: lean
    /// the pen over and the contact patch draws out along the lean.
    pub stretch: Option<Modulation>,
}

impl BrushModulations {
    /// The everyday brush: **size follows pressure**, linearly, all the way to nothing.
    /// Data rather than a wired-in rule, so it can be turned off or pointed somewhere
    /// else like any other mapping.
    ///
    ...
    /// that holds its width whatever the hand does asks for the empty set.
...
/// [`PaintEffect`](super::PaintEffect)'s rate and its ceiling, held with the effect they
/// modulate ([`PaintEffect::modulation`](super::PaintEffect::modulation)) so a mapping
/// cannot name a knob its brush does not have.
///
/// The ceiling costs what the rate does not: a segment's share of the stroke's coverage
/// has to be remembered at the segment's own ceiling, so a stroke that maps it carries
/// one more accumulator lane per touched tile (§6.2). A brush that leaves it unmapped
/// never pays for it.
#[derive(Copy, Clone, Debug, PartialEq, Serialize, Deserialize, Default, carbonite::Schema)]
pub struct PaintModulations {
    ...
    /// Scales [`PaintEffect::opacity`](super::PaintEffect::opacity) — how much of a full
    /// stroke this stretch of it is. Where a stroke covers a spot twice the spot shows
    /// the **larger** of the two ceilings (§6.2), so a light pass back over a heavy mark
    /// leaves it, and a heavy pass over a light one fills it in.
    #[serde(default)]
...
/// The pen mappings whose targets exist only while **working wet paint** (§6.2) — the
/// effect's own flow and ceiling, and the four rates of
/// [`BrushDynamics`](super::BrushDynamics). With the effect they modulate
/// ([`WetEffect::modulation`](super::WetEffect::modulation)), for
...
pub struct WetModulations {
    ...
    /// less.
    pub flow: Option<Modulation>,
    /// Scales [`WetEffect::opacity`](super::WetEffect::opacity) — the ceiling on what
    /// the brush **mints** (§6.2). The `charge` glob is scaled by the dial alone, being
    /// minted once before the pen has moved; moved paint is under no ceiling at all.
    #[serde(default)]
    pub opacity: Option<Modulation>,
...
mod tests {
    ...
    /// A modulation is a pure function of floats, so replay, goldens and peers agree on
    /// it — true only while it stays clear of the unspecified library transcendentals
    /// (`Modulation::shape`).
    #[test]
