...
//!
//! A fill is the **fifth thing a shape gesture can do**. Rect, ellipse and lasso
//! produce *coverage*, and [`SelectionMode`] is only the four ways that coverage can
//! land on the selection mask; landing it on the **paint** instead is
//! [`ShapeAction::Fill`], with the same shapes, the same analytic rasterizer and the
//! same feather.
//!
//! - **The selection bounds the fill.** §6.8 makes the mask the gate every tool acts
//!   through, so a fill is gated identically to a brush stroke. A fill with *neither*
//!   a bounded shape nor a bounded selection is refused (`stark-engine`'s
//!   `document::fill::plan` returns `None`), deterministically, so peers and replays
//!   agree.
//! - **A fill deposits paint, not color.** The parcel it lands is fully opaque paint
//!   of a real thickness — enough of it to *be* the coverage asked for
//!   ([`FillOp::opacity`]) — so a filled region takes the light, can be glazed over,
//!   and a lift brush can scrape it back. It stacks by the shared parcel law
//!   (`paint_common.wesl`), the law a stroke deposits through.
//!
//! Pure CPU geometry: `stark-engine`'s `document::fill::plan` decides *which* tiles,
//! and its `gpu::fill::FillRenderer` does the GPU work.
...
/// Largest number of paint tiles one fill may write — the same stance as
/// [`MAX_TRANSFORM_TILES`](super::transform::MAX_TRANSFORM_TILES). A fill that would
/// exceed it is refused whole rather than clipped, and being a pure function of the op
/// and the mask's tile set, refused identically everywhere.
...
/// question, so "exactly one is lit" is structural rather than a rule two pieces of
/// state have to be kept agreeing on.
...
/// One logged fill (§18.0.4): a region, and the parcel of paint to lay in it. Compact
/// enough for the action log and the wire, like [`SelectionOp`](super::SelectionOp) —
/// the shape travels, never the tiles, and every peer rasterizes it identically from
/// the same shader.
///
/// **Deserialization funnels through [`FillOp::with_paint`]**, which is what holds the
/// invariants the wire form does not: a fill lands paint whose mass is the *inverse*
/// of the coverage law (§6.1), so an opacity of 1.0000001 from a corrupt log is not a
...
pub struct FillOp {
    ...
    /// in `0..=1` — the only strength knob a fill has. The shader inverts §6.1's
    /// coverage law (`fill.wesl`) and lays fully opaque paint of exactly that mass, so
    /// the slider's number is the canvas's; partial coverage — a feathered edge, or
    /// the selection gating the fill — scales it. One opacity for the whole fill: a
...
impl FillOp {
    ...
    ///
    /// **At full opacity by construction**, which is why it takes no opacity
    /// parameter: this fill's region *is* the selection, so how strongly it lands is
    /// already written into the mask it comes through (§6.8), and dimming itself as
    /// well would apply the Opacity slider twice.
    ...
    /// gradient, the axis the ramp is read from (§22.4). What brings a canvas-space
    /// gesture into the layer's frame at mint (§14.12); feather, paint and opacity are
    /// not positions and ride through.
    ///
    /// Goes through [`Self::with_paint`], so the result holds the op's invariants
    /// however far the shift lands.
    ...
    /// carries a tile's write one band further. Deliberately tighter than
    /// `stark-engine`'s `Selection::plan` padding, which is sized so the *outline*
    /// pass can find a boundary by differencing: reusing it here would ring every fill
    /// with all-zero paint tiles that pollute `bounds` and hold pool memory.
    fn reach(&self) -> f32 {
        self.feather.max(1.0) * 0.5 + TILE_APRON as f32 + 1.0
    }
}

/// The canvas-space box a fill can write: its shape's own box, grown by the coverage
/// ramp (`FillOp`'s reach) and then by the apron band, since a box reaching into that
/// band still touches the neighbour.
///
/// `None` when the fill is bounded only by the selection — [`SelectionShape::All`], or
/// a lasso with no vertices — which `stark-engine`'s `document::fill::plan` then bounds
/// by the mask and the footprint has to claim as the whole layer.
///
/// **One box, quantized twice**, which is the whole point of this function.
/// `stark-engine`'s `document::fill::plan` turns it into the tiles a fill writes and
/// [`fill_rect`](super::footprint::fill_rect) into the tiles the action declares, and
/// those must be the same tiles: a footprint naming fewer than the plan writes is the
/// §12.6 under-claim, and it also leaves tiles behind on undo, since the undo diff is
/// bounded by the rect the action declared. The two quantize *separately* only because
/// they answer differently for a box that cannot be quantized at all — the footprint
/// claims everything, the plan refuses. What they must not do is derive the box
/// differently, which is why it is derived here.
...
/// `serde(from/into)` for the encoding, `carbonite(as)` for the schema (§8). A saved
/// fill's columns and field *names* come from here, and the rename back to `FillOp` is
/// what keeps a document from recording the mirror's name.
///
/// The conversion is `From` and never `TryFrom` by design: a hostile value is
/// *clamped* on the way in, not rejected, because a fill that will not open is worse
/// than one whose opacity was pulled into range.
...
mod tests {
    ...
    /// inverse runs to infinity at 1 — so a log carrying 1.0000001 is a `NaN` texel.
    #[test]
    ...
    /// there is no second layout to drift from and a field present on one side only
    /// will not compile. What is left to test is that the round trip is the identity
    /// on a normalized op.
    #[test]
