...
///
/// These are **characterisation** tests: the expectations are the current output, not
/// an independently-derived one, and two of them pin behaviour that is wrong — see
/// [`tests::a_const_derived_from_its_neighbours_is_skipped`]. A correctness fix is
/// supposed to fail them, and the diff is what it changed.
///
...
",
    ...
    /// The header, and the two shapes that reach it.
    ///
    /// **`DOUBLE` should not be here.** `Context::new` opens a *function* scope, and
    /// `wesl`'s evaluator only looks a name up in the module's declarations from a
    /// module scope — so a constant naming its neighbour fails to evaluate, where
    /// `consts::emit`'s own documentation says it comes out as the number the shader
    /// computes with. `@binding(SOME_CONST)` fails for the same reason. No `const` in
    /// the tree names another today, so nothing is missing from the real mirror.
    #[test]
    fn a_const_derived_from_its_neighbours_is_skipped() {
        let out = mirrors(
            &[Module::parse(
                "probe",
                r"
...
",
        ...
        assert_eq!(
            ...
            "// @generated by `stark-shaders-build` from the WESL sources — do not edit.\n\
             //\n\
             // Declarations discovery reached and could not spell in Rust:\n\
             //   `probe.wesl`'s `const DOUBLE` does not const-evaluate\n\
