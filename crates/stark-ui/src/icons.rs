//! The icons the controls wear, and why each one (§11, §25).
//!
//! **Which glyph a control wears is not a frontend's business.** It is a statement
//! about what the control *means* — that Add, Sub and Isect are one question answered
//! three ways, that a trash is the destructive control everywhere, that a funnel is
//! what a stack is seen *through* — and two apps disagreeing about that would be two
//! apps. So the catalog is here, prose and all, and what stays in each frontend is
//! how a glyph reaches the screen: inlined into the DOM on one side, rasterized to an
//! alpha mask and tinted on the other.
//!
...
//!
//! Every file paints with `fill="currentColor"`, which is what lets one file cover a
//! resting chip, a lit chip's white-on-blue and a disabled chip's fade. That is a
//! claim about files nobody reviews — a Phosphor download arrives with `fill="#000000"`
//! baked in, and an icon that keeps it looks *right* in a file browser and paints a
//! black glyph on near-black chrome. The test below checks the *directory* rather
//! than the table, so an icon added and not yet wired up is caught before it is ever
//! drawn.
//!
...
/// The SVG source for a file stem, whether or not any control wears it.
///
...
icons! {
    ...
    // The H hotkey's act (`stark_ui::commands::Command::MirrorView`): the same picture as
...
mod tests {
    ...
    /// browser and paints a black glyph on near-black chrome — or, in a frontend that
