//! The Gradients panel (§22.3) and the trace mode it arms (§22.2).
//!
//! The library rows live here; the *making* of one is a mode over the canvas,
//! like the transform: while armed, a full-viewport catcher owns the pointer,
//! the drag is collected in **canvas space** (so panning or zooming mid-trace
//! cannot corrupt it — `input::Nav` stays live on the catcher), and release
//! hands the polyline to [`gradients::capture`]. A drag too short to mean
//! anything ends the mode without a capture, so a stray click is its own undo.
//!
...
/// The trace mode's catcher and rubber line, mounted while the Gradients
/// panel's Trace is armed. The transform overlay's shape (§16.6): a
/// full-viewport catcher that owns the pointer, with `Nav` live on it so the
/// view stays reachable, and an SVG polyline of the trace so far.
#[component]
...
fn trace_line(points: &[Vec2], view: stark_core::ViewTransform) -> Element {
