...
| **transfer** | How a target's texels encode light (§6.5): the sRGB curve over `[0,1]`, the same curve continued past 1 and mirrored through 0 (extended sRGB), or none (linear scRGB). A fact about the *surface*, read off its configuration by the frontend and stated to the engine; never chosen from the format alone, since one fp16 format is read two ways. | [`stark-engine/src/gpu/composite/display.rs`](../crates/stark-engine/src/gpu/composite/display.rs) |
