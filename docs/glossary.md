...
| **backing** | The §15.5 underpainting: an `Everything` matte born at the bottom of the stack, under the painting. A *layer*, unlike the substrate color — paintable, movable, undoable. | [`stark-dioxus-frontend/src/panels/frame.rs`](../crates/stark-dioxus-frontend/src/panels/frame.rs) |
| **surface** | Reserved for four things that are not the canvas: wgpu's swapchain `Surface`, the warp mesh's mathematical surface (§16), a *UI surface* (a pointer-receiving region of the chrome), and a module's *public surface*. | [`stark-dioxus-frontend/src/render.rs`](../crates/stark-dioxus-frontend/src/render.rs) |
...
| Term | What it is | Defined in |
|---|---|---|
| **tip** | The brush at one instant: a position, a radius, an angle. `BrushShape` is the tip's shape. | [`stark-model/src/document/brush/mod.rs`](../crates/stark-model/src/document/brush/mod.rs) |
...
| **wet brush** | A brush whose effect is `Wet` (§6.2): it lays paint *and* works what is already there through the sequential lift/deposit loop. Its own effect rather than a `Paint` at other rates, because the two are different tools with different available features — a wet stroke mixes with the canvas and carries a reservoir, and its deposit is point-sampled where paint's is antialiased through the pixel-footprint filter. The variant *is* the render path: `Wet` loops, `Paint` and `Erase` sweep. | [`stark-model/src/document/brush/effect.rs`](../crates/stark-model/src/document/brush/effect.rs) |
| **eraser** | A brush whose effect is `Erase` (§6.12) — `BrushEffect`, the tool's identity as a sum type: the stroke removes *visible* opacity through the slab law, capped per stroke at the dial. Not the `lift` axis, which is a **scraper** — it takes the amount, conserving paint, and reads in height rather than in what the eye sees. | [`stark-model/src/document/brush/effect.rs`](../crates/stark-model/src/document/brush/effect.rs) |
| **liquify brush** | A brush whose effect is `Liquify` (§6.13): the stroke drags the picture itself — a homeomorphism of the canvas, kept as a displacement **field** and applied to the picture by one resample, structure carried rather than mixed. The domain's own word (Photoshop, Procreate, Clip Studio), kept because "warp" already belongs to the selection transform's mesh (§16.9); its per-texel motion is the **follow**, a fraction of the tip's travel. Not a smudge, which trades paint through a reservoir and conserves height. | [`stark-model/src/document/brush/effect.rs`](../crates/stark-model/src/document/brush/effect.rs) |
| **rise** (of a tip) | The shortest travel, in radii, over which a tip's coverage climbs by `WARP_CONTRACTION` (§6.13) — what the liquify step budget prices, so a step at strength 1 *is* the rise. Searched off the round tip's radial profile (`tips::round_rise`), bounded off a stamp's steepest texel step (`assets::mask_rise`), floored at one canvas texel by the budget. Not the **shoulder** (§6.2), which is the falloff's *width* and a lateral quantity: the rise is measured along the travel, because only the climb along the travel can fold the map. | [`stark-engine/src/gpu/stroke/budget.rs`](../crates/stark-engine/src/gpu/stroke/budget.rs) |
| **coverage prefix** | The prefix-τ's volume with the coverage integrated linearly instead of as optical depth (`assets::Integrand::Coverage`, §6.13): read at the prefix-τ's own binding by the liquify `warp` kernel, so a follow is the mask's mean over the pass times the travel. A round tip bakes it beside its prefix-τ; a stamp bakes it on the first liquify stroke that asks. | [`stark-engine/src/assets.rs`](../crates/stark-engine/src/assets.rs) |
| **liquify run** | The state a sequence of liquify strokes on a layer composes through (§6.13): the pristine **base** the picture is resampled from, the composed **field**, the tiles the run last **produced**, and the per-tile **reach** — a bound on the field that sizes what a stroke reads and caps where a run **re-bases**. Written only by liquify strokes; every other paint edit carries it forward, and the next liquify stroke tells whether it is still fresh by tile identity. | [`stark-engine/src/document/liquify.rs`](../crates/stark-engine/src/document/liquify.rs) |
...
| **nib** | Only ever a simile — "a chisel nib", "like a calligraphy nib" — describing a real instrument whose shape a brush emulates. The modeled thing is a **tip**. | [`stark-model/src/document/brush/mod.rs`](../crates/stark-model/src/document/brush/mod.rs) |

## Paint and compositing

| Term | What it is | Defined in |
|---|---|---|
| **height** | The amount of paint at a texel. The conserved channel (§6.1) — a smear moves it, never creates it. | [`stark-model/src/document/brush/effect.rs`](../crates/stark-model/src/document/brush/effect.rs) |
| **opacity** | Two things, and the context says which. *Per-unit* opacity: a material property of resident paint (a tile's color alpha), meeting height only in the slab law `1 − exp(−K · opacity · thickness)`. *The effect's* opacity (`BrushEffect::opacity`, §6.2/§6.12): the per-stroke ceiling on what a saturated stroke lays or removes — the digital artist's dial, beside the flow that is the rate. The brush color itself carries no alpha. | [`stark-model/src/document/brush/effect.rs`](../crates/stark-model/src/document/brush/effect.rs) |
...
| **ceiling lane** | The sums a pen-driven opacity accumulates in place of the one dial it no longer has: a stroke's mass and moment above each of two ceiling levels, from which the largest ceiling any paint at a texel asked for is read, averaged over the texel (§6.2, *The ceiling under the pen*). The sweep's fourth target, the erase sweep's second, and a region-sized lane drawn per segment on the stamp loop. | [`stark-shaders/src/shaders/stamp_common.wesl`](../crates/stark-shaders/src/shaders/stamp_common.wesl) |
...
| **pigment** | The Mixbox pair — three concentrations plus the **residual** beside them — as one value, and the name of the axis the three colour-space passes are linked along. Not *latent*, which is any space's three channels. | [`stark-shaders/src/shaders/lib/mixbox.wesl`](../crates/stark-shaders/src/shaders/lib/mixbox.wesl) |
...
| **transfer** | How a target's texels encode light (§6.5): the sRGB curve over `[0,1]`, the same curve continued past 1 and mirrored through 0 (extended sRGB), none at all (linear scRGB), or either of the two over Display P3's primaries. A fact about the *surface*, read off its configuration by the frontend and stated to the engine; never chosen from the format alone, since one fp16 format is read four ways. | [`stark-engine/src/gpu/composite/display.rs`](../crates/stark-engine/src/gpu/composite/display.rs) |
