...
| — | HDR output (§6.5) | **done** — the light above white on an HDR surface, in both frontends: the tonemap's knee at the display's headroom, the media and resolve passes encoding for the surface's transfer, an export SDR by construction, and a switch that previews it |
| — | Wide-gamut paint (§6.5) | **done, less the wheel** — `Srgb` is extended sRGB (the wire at 25, the file untouched), the transfer is odd and unbounded, the mid-pipeline floors moved into XYZ, Display P3 surfaces in both transfers, and the readout speaks `color(display-p3 …)`. The picker's *wheel* still passes `Gamut::Srgb` in both frontends because neither picture carrier can show more — a tagged `data:` URL on the web, a wide sprite or a small `WgpuSurface` natively, is what would move it. Wide-gamut **import** (a second picture kind, §19) and PQ surfaces remain |
