...
| — | HDR output (§6.5) | **stage 1 done** — the light above white on an HDR surface, in both frontends: the tonemap's knee at the display's headroom, the media and resolve passes encoding for the surface's transfer, an export SDR by construction, and a switch that previews it. Wide-gamut paint (a document-format change: `Srgb` widened, the picker refitted, the mid-pipeline clamps moved to XYZ), Display P3 and PQ surfaces, and a runtime retarget across displays remain |
