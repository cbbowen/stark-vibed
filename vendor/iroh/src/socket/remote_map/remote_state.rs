...
impl RemoteStateActor {
        ...
        loop {
            ...
            tokio::select! {
                ...
                Some(item) = maybe_next(self.state.address_lookup_stream.as_mut()), if self.state.address_lookup_stream.is_some() => {
                    ...
                    // STARK PATCH: lookups may introduce custom addrs while
                    // connections already exist; open their paths.
                    self.open_custom_paths();
                }
    ...
    async fn handle_message(&mut self, msg: RemoteStateMessage) {
        ...
        match msg {
            ...
            RemoteStateMessage::ResolveRemote(addrs, tx) => {
                ...
                // STARK PATCH: a dial may introduce custom addrs while
                // connections already exist; open their paths.
                self.open_custom_paths();
            }
    ...
    fn handle_msg_add_connection(
        ...
        self.trigger_holepunching();
        // STARK PATCH: custom addrs may already be known for this remote (e.g.
        // from the dial addr); open their paths on the new connection.
        self.open_custom_paths();
        self.select_path();
    ...
    /// STARK PATCH: opens paths for all known custom-transport addrs on all
    /// connections.
    ///
    /// Upstream iroh only establishes a custom path when it wins the initial
    /// dial race: holepunching opens IP paths, only relay paths are re-added
    /// post-handshake, and the path selector picks solely among already-open
    /// paths. A connection that lands on the relay therefore never migrates to
    /// a custom transport. Calling this whenever custom addrs or connections
    /// appear lets such connections open (and, via the selector, migrate to)
    /// custom paths — e.g. a WebRTC data channel bootstrapped over the relay.
    ///
    /// Dead custom addrs are harmless: the path never validates, gains no
    /// stats, and is never selected. `open_path_on_conn` skips addrs already
    /// open on a connection.
    fn open_custom_paths(&mut self) {
        if self.connections.is_empty() {
            return;
        }
        let custom_addrs: Vec<transports::FourTuple> = self
            .state
            .paths
            .addrs()
