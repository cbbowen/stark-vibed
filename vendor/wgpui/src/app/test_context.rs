...
/// A TestAppContext is provided to tests created with `#[wgpui::test]`, it provides
...
impl TestAppContext {
    /// Creates a new `TestAppContext`. Usually you can rely on `#[wgpui::test]` to do this for you.
    ...
    /// In an editor that binds those keys to the command palette, this types
    /// backspace via the palette. This also runs the background executor until it parks.
