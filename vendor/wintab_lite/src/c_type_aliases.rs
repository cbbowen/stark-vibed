...
/// A window handle.
///
/// STARK PATCH: added, so that `extern_function_types` need not reach for the
/// `windows` crate to name one type — see VENDORING.md. `isize` is what the crate's
/// own `raw-dylib` declaration in `extern_functions` already uses for the same
/// parameter, and what `windows`'s own `HWND` is on this platform, so the two agree
/// about the ABI.
pub type HWND = isize;
